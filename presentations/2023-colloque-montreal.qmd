...
- and with the aim of improving livability and health.

## 

The 15-min neighborhood^[Sometimes mislabeled as the 15-min city.] is powered by:
...
When combined with the environment, the 15-min ped shed is:
...
$$
A_i = \sum_{j \in \text{15-min ped shed}} O_j
$$

Keeping the environment constant, bigger ped sheds result in higher accessibility.

## 

The maximum ped shed area^[For a given walking speed] would be in a homogeneous, isotropic surface (i.e., a featureless plain)
...
Streetscapes limit where movement can happen, which inevitably leads to smaller ped sheds. 
...
How much do 15-min ped sheds vary in a typical Canadian city (Hamilton)?
...
Ped sheds calculated from zonal centroids using [`r5r`](https://ipeagit.github.io/r5r/) assuming a walking speed of 3.6 $km/h$.
...
Summary of ped shed areas (max. ped shed area is `r round(pi * max_shed_r^2 |> units::set_units(km^2), 2)` $km^2$).
...
Profiling the ped sheds based on their network attributes.
...
# Network attributes of ped sheds

## 

Re-examining the areas of the ped sheds.
...
Ped shed area as a function of network attributes.
...
- Ped shed area varies greatly within Hamilton.
- The size of ped sheds varies significantly with the attributes of the network.
- The effect is non-linear.
- Results suggest an optimal combination of attributes to maximize ped shed areas.

## Implications

- Typically suburban networks result in relatively small ped sheds.
