...
Bind the centroid and non-centroid dataframes (da and validations):
```{r}
walksheds <- bind_rows(walksheds_train,
                                walksheds_validation) |>
  mutate(origin_type = rep(c("centroid", "non-centroid"), each = nrow(walksheds_da)))
```
...
```{r}
sample_walkshed <- sample.int(nrow(walksheds), 1)

wsnet <- hamilton_edges |>
  st_filter(
    walksheds[sample_walkshed,],
    .predicate = st_intersects)
...
ggplot() +
  geom_sf(data = walksheds[sample_walkshed,]) +
  geom_sf(data = wsnet) +
  theme_void()
...
```{r}
walksheds_motif_3_count <- data.frame(matrix(0, nrow = nrow(walksheds), ncol = 16))

walksheds_motif_3_count <- walksheds_motif_3_count |>
  rename_with(~ paste0("iso_class_", c(0:15)), starts_with("X"))

DA <- numeric(length = nrow(walksheds))
TYPE <- numeric(length = nrow(walksheds))
```
...
for(i in 1:nrow(walksheds)){
  ...
  # Convert to sfnetwork
  wsnet <- as_sfnetwork(wsnet)
  walksheds_motif_3_count[i,] <- motifs(wsnet)
}

walksheds_motif_3_count <- walksheds_motif_3_count |>
  mutate(GeoUID = walksheds$GeoUID,
         Type = walksheds$Type,
         origin_type = walksheds$origin_type) |>
  select(GeoUID, Type, origin_type, starts_with("iso"))
...
Save the data object:
```{r}
save(walksheds_motif_3_count, 
     file = glue::glue(here::here(), "/data/walksheds_motif_3_count.rda"), 
     compress = "xz")
```

Summary of motif counts:
```{r}
summary(walksheds_motif_3_count)
```
...
wsnet <- hamilton_edges |>
  st_filter(
    walksheds |> filter(GeoUID == ID, origin_type == "centroid"),
    .predicate = st_intersects)
...
ggplot() + 
  geom_sf(data = walksheds |>
            filter(GeoUID == ID, origin_type == "centroid")) +
  geom_sf(data = wsnet |>
            activate(edges) |>
            st_as_sf()) +
  geom_sf(data = motifs_class_10 |>
